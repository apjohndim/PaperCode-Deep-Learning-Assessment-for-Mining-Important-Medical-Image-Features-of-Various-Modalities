...
from data_loader_ph2 import load_CTCOV, load_CTX,load_Cells,load_MPIP,load_break,load_cmx, load_leukemia, load_petlidc, load_skin, load_lung, load_colon, load_lymph, load_cervical,load_prostate
...
path = 'E:\\Biomarkers phase 3 VGG\\BreakHis dataset (Kaggel)\\'
...
#data, labels, labeltemp, image = load_lymph(path)
#data, labels, labeltemp, image = load_prostate(path)
#data, labels, labeltemp, image = load_cervical(path)
data, labels, labeltemp, image = load_break(path)
...
in_shape = (115,175,3) # must be same with image returned from load_[which data](path)
...
model = 'lvgg'
model3, predictions_all,predictions_all_num,test_labels,labels, auc, conf_final,final_score,mean_f1 = train(epochs,batch_size, model, in_shape, tune, classes, n_split)
